...
export async function GET(
  ...
  try {
    ...
    // Find grievance - case insensitive search for hex values
    // Escape special regex characters in grievanceId
    ...
    if (!grievance) {
      ...
      // Try to find any grievances for debugging
    ...
    if (!submission) {
      console.log('❌ Submission not found:', grievance.submissionId);
      return NextResponse.json(
    ...
    if (!test) {
      console.log('❌ Test not found:', grievance.testId);
      return NextResponse.json(
    ...
    if (!originalEvaluation) {
      console.log('❌ Original evaluation not found:', grievance.originalEvaluationId);
      return NextResponse.json(
    ...
    // Check if re-evaluation already exists
    ...
    if (grievance.reevaluationId) {
      ...
      console.log('ℹ️ Re-evaluation found:', reevaluation ? 'Yes' : 'No');
    }

    console.log('✅ Successfully fetched all grievance details');
